  ...
  return (
    <div className='flex-wrap lg:flex-nowrap flex gap-10' >
      <motion.div variants={fadeTop} {...motionStep} className='relative min-w-[200px]' >
        <div>
          {worked_data.map((e: WorkedTypes, i: number) =>
            <div
              ...
              className={cx(
                activeIndex === i && 'bg-red-600/10 text-red-700',
                'tab__item'
              )}
            >
              {e.label}
        ...
        <span
          className="absolute left-0 block w-1 bg-red-700 transition-all duration-300"
          ...
          <div key={i} >
            <motion.div variants={worked_1} {...motionStep} >
              <h1 className='text-xl font-medium text-white' >
                {title}
                <Link target='_blank' href={url} className='text-red-600 border-b border-b-red-600 border-dotted' > @{company} </Link>
              </h1>
              <p className='text-white font-medium mt-1' > {deadline} </p>
            </motion.div>
            ...
            <ul className='space-y-5 mt-10 max-w-xl' >
              {description.map((e: string, i: number) => (
                <motion.li key={i} variants={lAnimation[i]} {...motionStep} className='gap-1 flex text-slate-400 select-none' >
                  <span className=' min-w-10' ><IoMdArrowDropright className='text-red-700 text-[1.3rem]' /> </span>
                  {e}
                </motion.li>
              ))}
            </ul>
